...
public class FieldHelper {
    ...
    public static FieldHelper getInstance()
    {
        if( _instance == null )
        {
            _instance = SpringUtils.getBean(FieldHelper.class);
        }
        return _instance;
    }
     ...
     * @return
     */
    public static com.hlg.fcf.annotation.Field getFieldAnnotation(Class<?> clazz, String fieldName) {
        return getInstance()._getFieldAnnotation(clazz,fieldName);
    }


    @Cacheable(key = "#p0+ '_(anno)'+ #p1")
    public com.hlg.fcf.annotation.Field _getFieldAnnotation(Class<?> clazz,String fieldName){
        Field field = getInstance()._getField(clazz,fieldName);
        return field.getDeclaredAnnotation(com.hlg.fcf.annotation.Field.class);
    }
