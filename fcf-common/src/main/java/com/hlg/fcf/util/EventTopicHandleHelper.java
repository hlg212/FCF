...
import com.hlg.fcf.event.RemoteEventTopicHandler;
    ...
    private static List getHandlers()
    {
        if( handlers == null )
        {
            synchronized (EventTopicHandleHelper.class) {
                Collection<RemoteEventTopicHandler> coll = SpringUtils.getApplicationContext().getBeansOfType(RemoteEventTopicHandler.class).values();
