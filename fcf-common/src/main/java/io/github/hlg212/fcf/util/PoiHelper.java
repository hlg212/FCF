...
public class PoiHelper {
    ...
    private final static String SHEET_PREFIX = "Sheet";
    private final static String SHEET1= "Sheet1";

    public static File merageSheet(File ...files) throws Exception {
        if( files.length == 1)
        {
            ExceptionHelper.throwBusinessException("只有一个文件，没必要合并!");
        }
        InputStream ins[] = new InputStream[files.length];
        File file = File.createTempFile("merageSheet",".xlsx");
        byte bs[] = merageSheet(ins);
    ...
    public static byte[] merageSheet(InputStream ...ins) throws Exception {
        if( ins.length == 1)
        {
            ExceptionHelper.throwBusinessException("只有一个文件，没必要合并!");
        }
        ...
        {
            XSSFWorkbook copy = new XSSFWorkbook(ins[i]);
            StyleMapping mapping = copyCellStyle(copy,workbook);
            for( int j=0;j<copy.getNumberOfSheets();j++ )
            {
                Sheet srcSheet = copy.getSheetAt(j);
                Sheet descSheet = createSheet(workbook,srcSheet.getSheetName(),0);
                copySheet( srcSheet,descSheet,mapping);
            }
    ...
    public static File[] splitSheet(File file) throws Exception {

       return splitSheet(new FileInputStream(file),SHEET1);
    }

    public static File[] splitSheet(InputStream in) throws Exception {

        return splitSheet(in,SHEET1);
    }

    public static File[] splitSheet(InputStream in,String sheetName) throws Exception {
        XSSFWorkbook workbook = new XSSFWorkbook(in);
        File[] files = new File[workbook.getNumberOfSheets()];
        for(int i=0;i<files.length;i++ )
        {
            files[i] = File.createTempFile("split" + i,".xlsx");
            XSSFWorkbook split = new XSSFWorkbook();
            StyleMapping mapping = copyCellStyle(workbook,split);
            Sheet srcSheet = workbook.getSheetAt(i);
            Sheet descSheet = split.createSheet(sheetName == null ? srcSheet.getSheetName() : sheetName );
            copySheet( srcSheet,descSheet,mapping);
            split.write(new FileOutputStream(files[i]));
            split.close();
        }
        workbook.close();
        return files;
    ...
    private static Sheet createSheet(XSSFWorkbook workbook,String sheetName,int index)
    {
        if( sheetName.startsWith( SHEET_PREFIX ))
        {
        ...
        if( index > 0 )
        {
            sname = sheetName + "(" + index +")";
        }
        if( workbook.getSheet(sname) != null )
        {
            return createSheet(workbook,sheetName  , index + 1);
        }
     ...
     * 复制行
     */
    private static void copyRow(Row srcRow, Row desRow) {
     ...
     * 复制行
     */
    private static void copyRow(Row srcRow, Row desRow, boolean copyValueFlag) {
     ...
     * 复制行
     */
    private static void copyRow(Row srcRow, Row desRow, StyleMapping mapping) {
     ...
     * 复制行
     */
    private static void copyRow(Row srcRow, Row desRow, boolean copyStyleFlag, StyleMapping mapping) {
     ...
     */
    private static void copyCell(Cell srcCell, Cell desCell) {
     ...
     * @param copyValueFlag 控制是否复制数据
     */
    private static void copyCell(Cell srcCell, Cell desCell, boolean copyValueFlag) {
     ...
     * @param mapping       不同文件间复制时，如果要复制样式，必传，否则不复制样式
     */
    private static void copyCell(Cell srcCell, Cell desCell,  StyleMapping mapping) {
     ...
     * @param mapping       不同文件间复制时，如果要复制样式，必传，否则不复制样式
     */
    private static void copyCell(Cell srcCell, Cell desCell, boolean copyStyleFlag, StyleMapping mapping) {
     ...
     *   StyleMapping 两个文件中styleTable的映射关系
     * @see StyleMapping
     */
    private static StyleMapping copyCellStyle(Workbook srcBook, Workbook desBook){
        if (null == srcBook || null == desBook) {
            ExceptionHelper.throwServerException("源excel 或 目标excel 不存在");
        }
        if (srcBook.equals(desBook)) {
            ExceptionHelper.throwServerException("不要使用此方法在同一个文件中copy style，同一个excel中复制sheet不需要copy Style");
        }
        ...
        (srcBook instanceof XSSFWorkbook && desBook instanceof HSSFWorkbook)) {
            ExceptionHelper.throwServerException("不支持在不同的版本的excel中复制样式）");
        }
     ...
     */
    private static class StyleMapping {
        /**
         *
         */
        ...
        public short srcIndex(short desIndex) {
            if (desIndex < 0 || desIndex >= this.des2srcIndexMapping.length) {
                ExceptionHelper.throwServerException("索引越界：源文件styleNum=" + this.des2srcIndexMapping.length + " 访问位置=" + desIndex);
            }
         ...
         *   desIndex 目标excel中style的index
         */
        public short desIndex(short srcIndex) {
            if (srcIndex < 0 || srcIndex >= this.src2desIndexMapping.length) {
                ExceptionHelper.throwServerException("索引越界：源文件styleNum=" + this.src2desIndexMapping.length + " 访问位置=" + srcIndex);
            }
